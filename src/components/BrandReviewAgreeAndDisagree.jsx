...
} from "../slices/businessReviewAgreeAndDisAgreeApiSlice";
import { PulseLoader } from "react-spinners";

const BrandReviewAgreeAndDisagree = ({ rating }) => {
  ...
  const [createAgree, { isLoading: isCreateAgree }] =
    useCreateBuisnessAgreeMutation();
  ...
  return (
    <ul>
      ...
      {/* ////////////////////////////////////////////////// */}
      <li>
        {reviewAgreeLoading || isCreateAgree ? (
          <PulseLoader
            color="rgb(0 40 86 / 80%)"
            loading={reviewAgreeLoading || isCreateAgree}
            size={6}
          ...
          <>
            <img
              src={
                reviewAgree.find(
                  (currEl) => currEl?.user === userInfo?.user?._id
                )
                  ? likeActive
                  : likeImg
              }
      ...
      {/* ////////////////////////////////////////////////// */}
      <li>
        {reviewDisAgreeLoading || isDisAgree ? (
          <PulseLoader
            color="rgb(0 40 86 / 80%)"
            loading={reviewDisAgreeLoading || isDisAgree}
            size={6}
            cssOverride={{ width: "37px" }}
          />
        ) : (
...
export default BrandReviewAgreeAndDisagree;
