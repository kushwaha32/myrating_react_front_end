...
import { useUpdateBrandRatingMutation } from "../slices/brandRatingApiSlice";
...
const BrandRatingAndReviewUpdate = ({
  handleClose,
  currentBrand,
  currentRating,
}) => {
  const [updateBrandRating, { isLoading }] = useUpdateBrandRatingMutation();
  ...
  const handleAvgRating = useBrandAvgRatingWithSocket(
  ...
  const {
    ...
    onSubmit: async (value) => {
      try {
        ...
        console.log(data);
        // parallel api call
        const res = await updateBrandRating(data).unwrap();
  ...
  useEffect(() => {
    setFieldValue("currentBrand", currentBrand?._id);
    ...
    <div className="rr-model">
      ...
      <form onSubmit={handleSubmit} className="rr-model-ca">
        <div className="rr-model-c">
          <ImgWithSideCaption
            img={currentBrand?.brandImage}
            title={currentBrand?.brandName}
          >
            <span>{currentBrand?.industry?.name}</span>
          </ImgWithSideCaption>
          {/* give Rating */}
          <ReactStars
            count={5}
...
export default BrandRatingAndReviewUpdate;
