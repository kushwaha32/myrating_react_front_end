  ...
  return (
    <figure>
      <span
        ...
        style={handleProductImg(userInfo?.user?.brandProfile?.brandImage || manImg)}
      ></span>
      <figcaption className="profile-overview-ab head-with-profile">
        ...
        <Link
          to={`/business/${userInfo?.user?._id}/about`}
          className="head-with-profile-a head-with-profile-a-link text-capitalize"
        >
          {userInfo?.user?.brandProfile?.brandName}
        </Link>
        <span className="head-with-profile-b text-capitalize">{userInfo?.user?.brandProfile?.registeredAs?.name}</span>
