...
import useBusinessPreviousSteps from "./useBusinessPreviousSteps";

const useBrandOtpHandler = (
  setMinuts,
  minuts,
  seconds,
  setSeconds,
      ...
      if (minuts === 0 && seconds === 0) {
        ...
        if (otp.length !== 6) {
          ...
          const res = await signupOtpVerify({
            ...
            data: JSON.parse(localStorage.getItem("businessInfo")),
            contactNumberOtp: otp,
          }).unwrap();

          if (res?.status === "success") {
            toast.success("Contact no verified successfully");
            handlePreviousStep("mobile-no-e-mail-id-verification");
...
export default useBrandOtpHandler;
